...
/* ---------- Connection safety & device reset ---------- */

// Catch the most common mistake: pasting the Sheet link instead of the Apps Script /exec link.
window.addEventListener('submit', event => {
  if (event.target.id !== 'sheet-connection-form') return;
  const value = String(event.target.elements.endpoint?.value || '').trim();
  if (/^https:\/\/script\.google\.com\/.+\/exec$/.test(value)) return;
  event.preventDefault(); event.stopImmediatePropagation();
  showToast(value.includes('docs.google.com') ? 'That is the Sheet link. Paste the Apps Script Web app link that ends in /exec.' : 'Paste the Apps Script Web app link that ends in /exec.');
}, true);

// Only offer demo items on a device with no live Sheet.
ownerCatalogueV3 = function () {
  const testCount = products.filter(item => item.testItem || String(item.id || '').startsWith('TEST-')).length;
  if (!testCount) return ownerCatalogueV2();
  return ownerCatalogueV2().replace('<div class="quick-actions">', `<div class="test-catalogue-note"><b>Demo items on this device</b><span>${testCount} test products are still stored here.</span><button data-clear-test-catalogue>Remove test items</button></div><div class="quick-actions">`);
};

const ownerMoreWithReset = ownerMoreV2;
ownerMoreV2 = function () {
  const reset = `<button class="owner-lock bi-reset" data-reset-device>Clear data on this device</button>`;
  return ownerMoreWithReset().replace('<button class="owner-lock" data-owner-lock>', `${reset}<button class="owner-lock" data-owner-lock>`);
};

document.addEventListener('click', event => {
  if (!event.target.closest('[data-reset-device]')) return;
  event.preventDefault(); event.stopImmediatePropagation();
  if (!window.confirm('Clear all orders, products and settings stored on this device? Your Google Sheet is not touched. Your PIN and owner key are kept.')) return;
  const keep = ['treasures-owner-pin', 'treasures-sheets-owner-key', 'treasures-data-source'];
